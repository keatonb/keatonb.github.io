...
The original approach of the method was to plot a power spectrum with matplotlib (in log-log space, without axes), save it to disk as a png file, then read it back in as a 2D image and rescale it to 128x128 pixel size. The big slow-down here is having to write the images to disk and read them back in.  Inspecting the images, I was also surprised to see that there seems to be some undesired image artifacts of this process; in particular, there appeared to be a useless border along the edges of each image, some hints of grey appeared when reading in the otherwise black-and-white image, and then even more grey was introduced when scaling the image down to a smaller size. These bits of grey concerned me as they seemed uncontrolled and could potentially cause a loss of information in the images, though clearly not enough to derail the overwhelming success of the classifier. Still, they weren't what I imagined an ideal image representation would look like, so I coded up a much faster alternative.
